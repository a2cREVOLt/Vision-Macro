...
if exist "submacros\natro_macro.ahk" (
	if exist "submacros\AutoHotkey32.exe" (
		if not [%~3]==[] (
			set /a "delay=%~3" 2>nul
			echo Starting Vision Macro in !delay! seconds.
...
if "%exe_missing%" == "1" (
	...
	echo %red% 1. Disable any third-party antivirus software ^(or add the Vision Macro folder as an exception^)%reset%
	...
	echo:
	echo %red%Note: Vision Macro and AutoHotkey work with Microsoft Defender^^!%reset%
	echo:
		...
		if not [!zip!] == [] (
			...
			for %%a in ("Downloads","Downloads\Natro Macro","Desktop","Documents","OneDrive\Downloads","OneDrive\Downloads\Natro Macro","OneDrive\Desktop","OneDrive\Documents") do (
				if exist "%%~a\!zip!" (
					...
					echo %green%Unzip complete^^! Starting Vision Macro in 10 seconds.%reset%
...
echo %red%Unable to automatically extract Vision Macro^^!%reset%
